export const toggleMenu = (target, className) => {
  ...
  if (menuButtons) {
    ...
    //Programação Funcional - Imutabilidade, Funções de primeira ordem.
    ...
    menuButtons.forEach((menu) => {
      menu.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        //css - kebabcase Ex. transition-duration
        //js - camelCase Ex. transitionDuration
...
export const slider = (reference = "", className, total) => {
  ...
  let prevItem = !reference ? 0 : Number(reference - 1); //5 + 4 = 9
  ...
  console.log("largura viewport: innerWidth", window.innerWidth);

  if (Number(window.innerWidth) >= 768 && Number(window.innerWidth <= 1280)) {
    //tablet
    sectionHero.style.backgroundImage = `url('../assets/images/background-banner-tablet-${reference}.webp')`;
  } else if (Number(window.innerWidth) > 1280) {
    //desktop
    sectionHero.style.backgroundImage = `url('../assets/images/background-banner-desktop-${reference}.webp')`;
  } else {
    //mobile
    sectionHero.style.backgroundImage = `url('../assets/images/background-banner-smartphone-${reference}.webp')`;
  }
  ...
  console.log(reference, "referência");
};
